    ...
    <div className="mx-auto max-w-3xl px-4 pb-10 pt-4 sm:px-6">
      ...
      <section className="border-b border-border pb-5">
        ...
        <div className="mt-5 flex items-center gap-5 sm:gap-8">
          <div className="relative shrink-0"><Avatar name={displayProfile.full_name} url={avatarPreviewUrl ?? displayProfile.avatar_url} size={96} /><span className="absolute bottom-0 right-0 grid size-6 place-items-center rounded-full border-2 border-paper bg-primary text-primary-fg"><Pencil className="size-3" /></span></div>
        ...
        <div className="mt-4 flex gap-2"><Button className="flex-1" variant="outline" onClick={() => setShowEditor((open) => !open)}><Pencil className="size-4" /> {showEditor ? 'Close editor' : 'Edit profile'}</Button><Button className="flex-1" variant="outline" onClick={async () => { const url = window.location.href; try { if (navigator.share) await navigator.share({ title: `${displayProfile.full_name ?? 'CommunityHero'} profile`, url }); else if (navigator.clipboard) await navigator.clipboard.writeText(url); else throw new Error('clipboard unavailable'); setShareLabel('Link copied'); setTimeout(() => setShareLabel('Share profile'), 1800) } catch { setShareLabel('Copy link failed'); setTimeout(() => setShareLabel('Share profile'), 1800) } }}><Share2 className="size-4" /> {shareLabel}</Button></div>
      </section>
