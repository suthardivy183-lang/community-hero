...
export function KioskPage() {
  return <main className="min-h-screen bg-paper px-4 py-8 sm:px-8"><div className="mx-auto max-w-4xl">
    ...
    <div className="mt-8 grid gap-4 md:grid-cols-3"><Option icon={<Mic />} title="Voice first" body="Use the grievance assistant to dictate in English, हिन्दी, or ગુજરાતી." action={<Link to="/grievance"><Button size="lg" className="w-full">Start grievance</Button></Link>} /><Option icon={<Globe2 />} title="Citizen tracking" body="Use a complaint number to open the grievance and follow status updates." action={<Link to="/map"><Button size="lg" variant="outline" className="w-full">Open map</Button></Link>} /><Option icon={<Accessibility />} title="Accessible access" body="Large text and low-bandwidth settings are available from the accessibility button." action={<Link to="/"><Button size="lg" variant="outline" className="w-full">Home</Button></Link>} /></div>
