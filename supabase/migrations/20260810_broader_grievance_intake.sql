...
create policy "incident members can read incidents" on public.infrastructure_incidents for select to authenticated
using (exists (
...
create policy "incident members can read memberships" on public.infrastructure_incident_reports for select to authenticated
using (exists (
...
declare
  ...
  v_anchor public.issues%rowtype;
  v_incident_id uuid;
begin
  ...
  if v_anchor.is_confidential and v_anchor.reporter_id <> auth.uid() and public.current_role() not in ('authority', 'supervisor', 'superadmin') then
    raise exception 'The comparison complaint is not visible to you';
  end if;
  ...
  if v_issue.geom is null or v_anchor.geom is null or not st_dwithin(v_issue.geom::geography, v_anchor.geom::geography, 200) then
    raise exception 'Only nearby complaints can be grouped';
  end if;
  ...
  if v_incident_id is null then
    ...
    values (
      ...
      st_y(v_anchor.geom::geometry),
      st_x(v_anchor.geom::geometry)
    )
    returning id into v_incident_id;
    insert into public.infrastructure_incident_reports (incident_id, issue_id) values (v_incident_id, v_anchor.id);
  end if;

  if exists (select 1 from public.infrastructure_incident_reports where issue_id = v_issue.id and incident_id <> v_incident_id) then
    raise exception 'This complaint is already grouped under another incident';
  end if;

  insert into public.infrastructure_incident_reports (incident_id, issue_id)
  values (v_incident_id, v_issue.id)
  on conflict (issue_id) do update set incident_id = excluded.incident_id;
